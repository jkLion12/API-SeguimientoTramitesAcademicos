...
require_once 'public/usuario.class.php';
...
if($_SERVER['REQUEST_METHOD'] == "GET"){
    ...
    if(isset($_GET["page"])){
        ...
        $_id = $_GET['id'];
        $datosUsuario = $_usuario->obtenerUsuario($_id);
        header("Content-Type: application/json");
        echo json_encode($datosUsuario);
        http_response_code(200);
    }else if(isset($_GET['codusuario'])){
        $_codusuario = $_GET['codusuario'];
        $datosUsuario = $_usuario->buscarUsuario($_codusuario);
        header("Content-Type: application/json");
        echo json_encode($datosUsuario);
        http_response_code(200);
    }
    ...
    if(isset($_GET['id'])){
        $_id = $_GET['id'];
        $datosArray = $_usuario->delete($_id);
        header("Content-Type: application/json");
        echo json_encode($datosArray);
        http_response_code(200);
    } else {
