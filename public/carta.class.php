...
class carta extends conexion{
    ...
    public function listaCarta($pagina = 1){
        $cantidad = 100000;
        $inicio = ($pagina - 1) * $cantidad;

        $query = "SELECT * FROM " . $this->table . " ORDER BY id_carta DESC LIMIT $inicio, $cantidad";
    ...
    public function obtenerCarta(){
        $query = "SELECT id_carta, nro_correlativo FROM " . $this->table . " ORDER BY id_carta DESC LIMIT 1";
        //print_r($query);
        //$datos = parent::obtenerDatos($query);
    ...
    public function post($json){
        ...
        if(!isset($datos['token'])){
            ...
            if($arrayToken){
                if(!isset($datos['id_anio_trabajo']) || !isset($datos['nro_correlativo']) || !isset($datos['detalle'])){
    ...
    private function insertarCarta(){
        $query = "INSERT INTO " . $this->table . " (id_anio_trabajo, nro_correlativo, detalle)
        values
        ('" . $this->id_anio_trabajo . "','" . $this->nro_correlativo . "','" . $this->detalle . "')";
        //print_r($query);
    ...
    private function modificarCarta(){
        $query = "UPDATE " . $this->table . " SET nro_correlativo = '" . $this->nro_correlativo .  
